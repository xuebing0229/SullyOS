...
test("keeps character, user, and vibe reference settings aligned", () => {
  const parameters = applyPreciseReference(
    { width: 1024, height: 1024 },
    [
      ...
      { imageBuffer: Buffer.from("vibe"), type: "style", strength: 0.45, fidelity: 0.8 }
    ]
  );
  ...
  assert.deepEqual(parameters.director_reference_images, [
    ...
    Buffer.from("vibe").toString("base64")
  ]);
  assert.deepEqual(parameters.director_reference_descriptions.map(item => item.caption.base_caption), ["character", "character&style", "style"]);
  assert.deepEqual(parameters.director_reference_information_extracted, [1, 1, 1]);
  assert.deepEqual(parameters.director_reference_strength_values, [0.75, 0.6, 0.45]);
  assert.deepEqual(parameters.director_reference_secondary_strength_values, [0.15, 0.3, 0.2]);
});
