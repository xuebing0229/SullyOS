...
const MessageItem = React.memo(({
    ...
    if (m.type === 'app_memory_card') {
        ...
        const isStoryTheater = md.sourceApp === 'story_theater';
        const sourceTitle = isSimulator ? '万象匣 · 记忆卡' : isStoryTheater ? '剧情剧场 · 记忆卡' : '素页同栖 · 记忆卡';
        const sourceIcon = isSimulator ? '◇' : isStoryTheater ? '▣' : '⌑';
        ...
        return commonLayout(
            <div className="w-64 overflow-hidden rounded-xl border border-amber-300/40 bg-gradient-to-br from-[#fff8e8] to-[#f4ecff] shadow-[0_4px_16px_rgba(80,60,120,0.14)]">
                <div className="flex items-center gap-2 border-b border-amber-300/30 px-3 py-2">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="text-[9px] font-bold tracking-[0.18em] text-violet-600/70">{sourceTitle}</div>
                ...
                <div className="border-t border-amber-300/20 px-3 py-1.5 text-[9px] italic text-stone-500">{isStoryTheater ? '记住的是共同演过，不把剧情当现实' : '已由你确认写入主聊天与主记忆'}</div>
            </div>
        );
    }
