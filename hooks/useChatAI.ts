...
**每次都必须返回完整的 buffs 快照：已消退的旧 buff 必须从数组中删除；全部消退时明确返回 []。不得因为 "changed": false 而省略 buffs。**
**只有在角色情绪状态、对对方的情绪感知和用户称谓都无需更新时，才返回 "changed": false；它只是说明字段，buffs 仍以本次返回的完整快照为准。**
