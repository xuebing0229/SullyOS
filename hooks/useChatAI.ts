...
**每次都必须返回“当前仍有效”的 buffs 列表：已消退的旧 buff 必须从数组中删除；全部消退时明确返回 []。这是为了让客户端知道哪些情绪已经消失，不代表旧 buff 的 name / label / description 必须照抄。**
**只有在角色情绪状态、对对方的情绪感知和用户称谓都无需更新时，才返回 "changed": false；它只是说明字段，buffs 仍以本次返回的当前状态为准。**
