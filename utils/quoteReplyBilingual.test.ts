...
describe('cleanApiMessages 双语剥离', () => {
    ...
    it('user 消息夹带 %%BILINGUAL%% 时不整段截断，标记后的回复保留', () => {
        const cleaned = cleanApiMessages([
            {
                role: 'user',
                content: `[用户引用了你之前说的「${BILINGUAL_CHAR_MSG}」，并回复了 ↓]\n${USER_REPLY}`,
            },
        ]);
        expect(cleaned[0].content).not.toMatch(/%%bilingual%%/i);
        expect(cleaned[0].content).toContain(USER_REPLY);
    });
});
