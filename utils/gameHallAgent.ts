...
export async function planGameHallTurn(input:{apiConfig:APIConfig;char:CharacterProfile;userProfile:UserProfile;mode:GameHallCompanionMode;userText:string;state?:NormalizedCedarGameState;actionTools:McpToolDef[];sessionId:string}):Promise<{reply:string;pending?:GameHallPendingAction;memorySignal?:GameHallMemorySignal}> {
  ...
  const prompt=`你是 ${char.name}，正在 SullyOS 游戏厅陪 ${userProfile?.name||'用户'} 玩 Cedar Toy。保持角色人设：${char.systemPrompt||char.description||''}\n模式：${mode}。当前游戏状态摘要：${state?.summary||'尚未读取'}\n用户说：${userText}\n行动工具真实 schema：${JSON.stringify(toolSchemas)}\n只输出 JSON：{"reply":"给用户的自然回复","action":null 或 {"toolName":"必须来自工具清单","args":{},"reason":"原因"},"memorySignal":null 或 {"category":"分类","summary":"不含密钥或原始 MCP JSON 的摘要","signals":[],"level":0到3}}。只有边界、承诺、偏好、关系变化、强情绪、共同命名/里程碑、持续目标等才给 memorySignal，普通流水为 null。observe 模式 action 必须 null。不得猜测 schema 中缺失的信息；不能填满全部 required 时 action 必须 null。`;
  ...
  if(!res.ok) throw new Error(`游戏厅 Agent HTTP ${res.status}`); const data=await safeResponseJson(res); const parsed=parseAgentJson(data?.choices?.[0]?.message?.content||''); const stripped=stripGameHallMemorySignals(String(parsed?.reply||'我在看着这局，先别急着动。').slice(0,2400)); const reply=stripped.visibleText.slice(0,2000); const memorySignal=(parsed?.memorySignal&&typeof parsed.memorySignal==='object'&&!('secret' in parsed.memorySignal)?parsed.memorySignal:stripped.signals[0]) as GameHallMemorySignal|undefined;
  if(mode==='observe'||!parsed?.action) return {reply,memorySignal}; const tool=actionTools.find(t=>t.name===parsed.action.toolName); const args=parsed.action.args; if(!tool||!args||typeof args!=='object'||Array.isArray(args)||!canCallWithoutGuessing(tool,args)) return {reply,memorySignal};
  return {reply,memorySignal,pending:{id:gameHallId('ghaction'),sessionId,charId:char.id,toolName:tool.name,args,reason:String(parsed.action.reason||'角色建议执行此行动').slice(0,500),stateHash:state?.stateHash,status:'pending',createdAt:Date.now(),updatedAt:Date.now()}};
}
